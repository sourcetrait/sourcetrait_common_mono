...
impl Er {
    ...
    const fn bridge_err(&self) -> agnostic::BridgeErr {
        match self {
            Self::chown => agnostic::BridgeErr::File,
            Self::fchmodat => agnostic::BridgeErr::File,
            Self::listxattr => agnostic::BridgeErr::File,
            Self::getxattr => agnostic::BridgeErr::File,
            Self::setxattr => agnostic::BridgeErr::File,
            Self::utimensat => agnostic::BridgeErr::File,
        }
    }
    ...
    #[inline]
    pub(crate) fn bridge_err_unknown<T>(self, opts: &FsOptions) -> agnostic::BridgeResult<T> {
        let noun = self.bridge_err();
        self.err_unknown(opts)
            .map_err(|e| agnostic::BridgeError::sys_call(noun, e))
    }
    ...
    pub(crate) fn bridge_lasterr<T>(self, opts: &FsOptions) -> agnostic::BridgeResult<T> {
        let noun = self.bridge_err();
        self._err(opts)
            .map_err(|e| agnostic::BridgeError::sys_call(noun, e))
    }
    ...
    pub(crate) fn bridge_lasterr_unsupported_ok_if(self, unsupported_ok: bool, opts: &FsOptions) -> agnostic::BridgeResult<()> {
        let noun = self.bridge_err();
