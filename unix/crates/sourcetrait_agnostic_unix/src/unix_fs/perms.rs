...
pub fn chown<P>(dst: P, uid: Option<UID>, gid: Option<GID>, opts: &FsOptions) -> agnostic::BridgeResult<()>
where
    P: AsRef<Path>,
{
    ...
    let dst_cstr = CString::new(dst.as_os_str().as_bytes())
        .map_err(|_| agnostic::BridgeError::String)?;
...
pub fn chmod<P>(dst: P, mode: UnixFileMode, opts: &FsOptions) -> agnostic::BridgeResult<()>
where
    P: AsRef<Path>,
{
